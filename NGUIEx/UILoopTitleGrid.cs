...
public class UILoopTitleGrid : UIWidgetContainer {
    ...
    public int m_dataArrangeNum = 0;                               //实际数据的行列数量
    ...
    public void MoveToIndex(int index) {
        index = (int)((float)index / (float)maxPerLine);//Mathf.CeilToInt((float)index / (float)maxPerLine);
        MoveRelative(index);
    }
    ...
    public void RefreshData() {
        ...
        if (preIsLoop && curIsLoop) {
            ...
            Vector2 cr = m_panel.clipOffset;
            Vector3 pos = m_panel.transform.localPosition;
            ResetToBegin();
            m_panel.clipOffset = cr;
    ...
    public Transform GetSelectOne(int gridIndex, int lineIndex) {
        foreach (var col in m_items)
        {
            if (col == null)
            {
                continue;
            ...
            foreach (var item in col)
            {
                ...
                if (item.gridIndex == gridIndex && item.lineIndex == lineIndex)
                {
    ...
    public void SelectOne(int gridIndex, int lineIndex) {
        foreach (var col in m_items) {
            if (col == null) {
                continue;
            }
    ...
    private void CalDistance(float distance, out float targetDis, out int index) {
        ...
        index = m_datas.Count;
        if (this.m_datas.Count > 0 && this.m_datas[0] is IParamData) {
            if (m_moveType == UIScrollView.Movement.Horizontal) {
                for (int i = 1; i < m_datas.Count; i++) {
                    if ((m_datas[i] as IParamData).GetPosx() >= targetDis) {
                        index = i - 1;
                        break;
                    }
                }
                ...
                for (int i = 1; i < m_datas.Count; i++) {
                    if ((m_datas[i] as IParamData).GetPosy() >= targetDis) {
                        index = i - 1;
                        break;
                    }
                }
    ...
    private void SlotData(IList datas) {
        if (datas == null)
        {
            //Debug.LogException(new Exception("绑定数据不能为空"));
        }
        m_datas = datas;
        if (itemTemplate == null)
        {
            //Debug.LogException(new Exception("模板不能为空"));
        }
    ...
    private void CheckDragBack() {
        ...
        // 面板没被占满拖拽回滚
        if (!m_scrollView.disableDragIfFits && m_dataArrangeNum < m_fillCount && !FullRegion()) {
            m_scrollView.onMomentumMove = OnMoveBack;
            m_scrollView.onDragFinished = OnMoveBack;
        }
    }

    private bool FullRegion() {
        return m_scrollView.bounds.size.y >= m_panel.baseClipRegion.w;
    }
